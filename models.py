...
class RmetricDB(db.Model):
    ...
    route_id = db.Column(db.Integer,
                         db.ForeignKey('route.route_id'))  # [ref: > route.route_id] //какому маршруту принадлежат
...
class PauseDB(db.Model):
    """
    Пауза.
    Например, сломался станок и мы вот ничего не можем поделать с этой партией.
    Мы ставим временно эту партию на паузу.
    Время паузы задаётся в минутах (в таблице по крайней мере)
    """
...
class LogBatchesLagDB(db.Model):
    """
        ...
        metrics - метрика, на сколько отстаёт партия, например, 10 - сильно опережает.
        critics - критика того, на сколько отстаёт партия, напримр, "сильно опережает".
        time_left - сколько времени осталось до окончания, в минутах.
    """
    __tablename__ = 'log_batches_lag'
    log_id = db.Column(db.Integer(), primary_key=True, autoincrement=True)
    batch_id = db.Column(db.Integer(), db.ForeignKey('batch.batch_id'))
    time = db.Column(db.DateTime())
    metrics = db.Column(db.Float())
    critics = db.Column(db.Text())
    time_left = db.Column(db.Integer())


class ChangeCountPartsInBatch(db.Model):
    """
    Таблица изменений количества деталей в партии.

    Например, мы делали партию и часть деталей в ней уже бракованные.
    Мы хотим считать оставшееся время уже не от 100 деталей, а от 95.
    """
    __tablename__ = 'change_count_parts_in_batch'
    change_id = db.Column(db.Integer(), primary_key=True, autoincrement=True)
    batch_id = db.Column(db.Integer(), db.ForeignKey('batch.batch_id'))
    now_count = db.Column(db.Integer())
